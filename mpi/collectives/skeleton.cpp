...
int main(int argc, char *argv[]) {
  ...
  //   MPI_Gatherv(sendbuf.data(), counts[rank], MPI_INT, recvbuf.data(),
  //   counts,
  //               displs, MPI_INT, 2, MPI_COMM_WORLD);
...
void print_buffers(std::vector<int> &buffer) {
  ...
  MPI_Gather(buffer.data(), buffersize, MPI_INT, printbuffer.data(), buffersize,
