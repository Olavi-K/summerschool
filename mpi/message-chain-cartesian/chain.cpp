...
int main(int argc, char *argv[]) {
  ...
  MPI_Cart_create(MPI_COMM_WORLD, ndims, dim, period, 1, &cart_comm);
  ...
  MPI_Sendrecv(message.data(), message.size(), MPI_INT, destination,
               cart_id + 1, receiveBuffer.data(), receiveBuffer.size(), MPI_INT,
               source, cart_id, cart_comm, &status);
