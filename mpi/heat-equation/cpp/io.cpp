...
void write_field(const Field& field, const int iter,
  ...
  if (0 == parallel.rank) {
    ...
    for (int i = 0; i < field.nx; i++)
      for (int j = 0; j < field.ny; j++) full_data(i, j) = field(i + 1, j + 1);

    // Receive data from other ranks
    for (int p = 1; p < parallel.size; p++) {
      MPI_Recv(tmp_mat.data(), field.nx * field.ny, MPI_DOUBLE, p, 22,
    ...
    filename_stream << "heat_" << std::setw(4) << std::setfill('0') << iter
                    << ".png";
    ...
    // Send data
    for (int i = 0; i < field.nx; i++)
      for (int j = 0; j < field.ny; j++) tmp_mat(i, j) = field(i + 1, j + 1);

    MPI_Send(tmp_mat.data(), field.nx * field.ny, MPI_DOUBLE, 0, 22,
             MPI_COMM_WORLD);
  }
}
...
void read_field(Field& field, std::string filename,
  ...
  if (0 == parallel.rank) {
    ...
    for (int i = 0; i < nx_full; i++)
      for (int j = 0; j < ny_full; j++) file >> full(i, j);

    for (int i = 0; i < field.nx; i++)
      for (int j = 0; j < field.ny; j++) inner(i, j) = full(i, j);

    // // Send data to others
    // for (int p = 1; p < parallel.size; p++) {
    //   MPI_Send(full.data(p * field.nx, 0), field.nx * field.ny, MPI_DOUBLE,
    //   p,
    //            22, MPI_COMM_WORLD);
    //   }
  } else {
    // MPI_Recv(inner.data(), field.nx * field.ny, MPI_DOUBLE, 0, 22,
    //          MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  }

  file.close();

  MPI_Scatter(full.data(), field.nx * field.ny, MPI_DOUBLE, inner.data(),
              field.nx * field.ny, MPI_DOUBLE, 0, MPI_COMM_WORLD);

  // Copy to the array containing also boundaries
  for (int i = 0; i < field.nx; i++)
    for (int j = 0; j < field.ny; j++) field(i + 1, j + 1) = inner(i, j);
