...
/*
   for those unfamiliar with c++ templates, using the coalesced boolean in the
   definition of the template is equivalent to create the following two
   different functions. at compile time, according to the value of the bool
   variable (which must be known), the compiler will select the correct version
   of the function. The if constexpr is indeed evaluated at compile time, and is
   equivalent to macros but is not a blind sostutution since the compiler will
   check that the values are correct. if you use cpp for your code, consider
   using constexpr instead of macros for constants and so on.
 */
...
__global__ void hipKernel_uncoalesced(int* const A, const int size)
{
  ...
  const int my_start = idx < num_thread_with_extra_point ? idx *
(num_points_per_thread + 1) : num_thread_with_extra_point *
(num_points_per_thread+1) + (idx-num_thread_with_extra_point)
*num_points_per_thread; const int my_end = idx < num_thread_with_extra_point ?
my_start + num_points_per_thread + 1 : my_start + num_points_per_thread; for(int
i=my_start; i<my_end; ++i) A[i] = A[i] + i;
}*/

template <bool coalesced>
__global__ void hipKernel(const int *const A, const int size,
                          const int *const B, int *const C) {
  const int idx = blockIdx.x * blockDim.x + threadIdx.x;
  const int num_workers = blockDim.x * gridDim.x;
  if constexpr (coalesced) {
    for (int i = idx; i < size; i += num_workers) C[i] = A[i] + B[i];
  } else {
    const int num_points_per_thread = size / num_workers;
    const int num_thread_with_extra_point = size % num_workers;
    const int my_start =
        idx < num_thread_with_extra_point
            ? idx * (num_points_per_thread + 1)
            : num_thread_with_extra_point * (num_points_per_thread + 1) +
                  (idx - num_thread_with_extra_point) * num_points_per_thread;
    const int my_end = idx < num_thread_with_extra_point
                           ? my_start + num_points_per_thread + 1
                           : my_start + num_points_per_thread;
    for (int i = my_start; i < my_end; ++i) C[i] = A[i] + B[i];
  }
}
...
void noRecurringAlloc(int nSteps, int size) {
  ...
  HIP_ERR(hipMalloc(&d_A, sizeof(int) * size));
  HIP_ERR(hipMalloc(&d_B, sizeof(int) * size));
  HIP_ERR(hipMalloc(&d_C, sizeof(int) * size));
  for (unsigned int i = 0; i < nSteps; i++) {
    // Launch GPU kernel
...
void recurringAllocNoMemPools(int nSteps, int size) {
  ...
  HIP_ERR(hipStreamSynchronize(0));
  // Check results and print timings
  checkTiming("recurringAllocNoMemPools",
...
void recurringAllocMallocAsync(int nSteps, int size) {
  ...
  for (unsigned int i = 0; i < nSteps; i++) {
    ...
    HIP_ERR(hipMallocAsync(&d_A, sizeof(int) * size, stream));
    HIP_ERR(hipMallocAsync(&d_B, sizeof(int) * size, stream));
    HIP_ERR(hipMallocAsync(&d_C, sizeof(int) * size, stream));

    // Launch GPU kernel
    hipKernel<coalesced>
        <<<gridsize, blocksize, 0, stream>>>(d_A, size, d_B, d_C);
  ...
  checkTiming("recurringAllocMallocAsync",
              (double)(clock() - tStart) / CLOCKS_PER_SEC);
}
