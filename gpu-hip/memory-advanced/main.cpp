...
__global__ void simple_kernel(mem_wrapper<int>::dev_side data) {
  if (threadIdx.x < data.num_elems) data.dev_ptr[threadIdx.x] = threadIdx.x;
}

__global__ void complex_kernel(example::dev_side data) {
  if (threadIdx.x < data.content.intcont
                        .num_elems)  // struct.tuple.field_in_tuple.mem_wrapper
    printf("Hello from thread %d, numelems is %d, my int value is %d \n",
           static_cast<int>(threadIdx.x),
           static_cast<int>(data.content.intcont.num_elems),
           data.content.intcont.dev_ptr[threadIdx.x]);

  if (threadIdx.x < data.content.floatcont.num_elems)
    printf("Hello from thread %d, numelems is %d, my float value is %f \n",
           static_cast<int>(threadIdx.x),
           static_cast<int>(data.content.floatcont.num_elems),
           data.content.floatcont.dev_ptr[threadIdx.x]);
}

int main() {
  // 1 layer wrapper example: easier (this is just a "RAII" wrapping of the
  // data, no more complexity involved.) it has to be managed manually
  ...
  // which makes the functions more readable. it is however more complex to
  // setup and manage
  ...
  // use a fold expression to copy all the data on the host side of the mem
  // wrappers
  std::apply(
      [&](auto&&... x) {
        std::apply([&](auto&&... y) { (y.copy_in(x.data()), ...); },
                   data_struct.content);
      },
      c);
  // copy to device
