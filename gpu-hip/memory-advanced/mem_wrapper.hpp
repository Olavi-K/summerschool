...
// wrapper template to manage arrays on the device. it is in charge of managing
// them as RAII, with the malloc done on the constructors and the free done in
// the destructor. It will only use async operations to perform memcpy from host
// to device and vice versa, for this reason all the memcpy and memset function
// will require a stream parameter operations with 0 elements are wrong. this
// will not be enforced, and if an operation is done with that value is
// performed on the whole array as it is used as default value for the functions
...
struct mem_wrapper {
  ...
  ~mem_wrapper() {
    ...
    if (host_ptr != nullptr) HIP_ERRCHK(hipHostFree(host_ptr));
    if (dev_ptr != nullptr) HIP_ERRCHK(hipFree(dev_ptr));
  }
  ...
  // to simplify the usage of these structures we also provide "update host" and
  // other ancillary functions. Two function to copy in-out are provided: one is
  // "punctual", in the sense that copies a num of elems that NEEDS to be
  // specified at a given offset (that MUST be provided) The other will just
  // copy "all", which means that is up to the programmer to be sure that the
  // size of the source pointer is the same of num_elems of this item. cpy in
  // and out will work with the HOST side of the mem wrapper!
  void copy_in(const T* __restrict__ other, const std::size_t offset,
  ...
  void copy_out(T* __restrict__ other, const std::size_t offset,
                const std::size_t num_elem) const {
  ...
  void cpy_to_host(const hipStream_t local_stream = 0) {
    HIP_ERRCHK(hipMemcpyAsync(host_ptr, dev_ptr, num_elems * sizeof(T),
                              hipMemcpyDeviceToHost, local_stream));
  }
  // move h2d (size)
  void cpy_to_dev(const hipStream_t local_stream = 0) {
    HIP_ERRCHK(hipMemcpyAsync(dev_ptr, host_ptr, num_elems * sizeof(T),
  ...
  void memset_dev(const hipStream_t local_stream = 0, const T value = 0) {
    ...
    HIP_ERRCHK(
        hipMemsetAsync(dev_ptr, value, num_elems * sizeof(T), local_stream));
  }
