...
__global__ void hipKernel(Example *const d_ex) {
  ...
  if (thread < d_ex->size) {
    printf("x[%d]: %.2f, idx[%d]:%d/%d \n", thread, d_ex->x[thread], thread,
           d_ex->idx[thread], d_ex->size - 1);
  }
}
...
void runDeviceUnifiedMem() {
  ...
  hipMallocManaged(&ex->x, ex->size * sizeof(float));
  hipMallocManaged(&ex->idx, ex->size * sizeof(int));
  ...
  hipKernel<<<gridsize, BLOCKSIZE, 0, 0>>>(ex);
...
Example *createDeviceExample(Example *ex) {
  ...
  Example *d_ex;
  hipMalloc(&d_ex, sizeof(Example));

  // #error Allocate device struct members
  float *d_x;
  int *d_idx;
  hipMalloc(&d_x, ex->size * sizeof(float));
  hipMalloc(&d_idx, ex->size * sizeof(int));
...
void runDeviceExplicitMem() {
  ...
  printf("\nDevice (ExplicitMem):\n");
  const int gridsize = (ex->size - 1 + BLOCKSIZE) / BLOCKSIZE;
  hipKernel<<<gridsize, BLOCKSIZE, 0, 0>>>(d_ex);
  hipStreamSynchronize(0);
