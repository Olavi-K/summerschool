...
void case_3(hipEvent_t *start_event, hipEvent_t *stop_event,
  ...
  // Distribute D-to-H memcopy and and record stop_events for 'n_streams'
  // streams
  for (int i = 0; i < n_streams; ++i) {
    int offset = i * stream_size;
    hipMemcpyAsync(&a[offset], &d_a[offset], stream_bytes,
...
int main() {
  ...
  for (int i = 0; i < n_streams; ++i) hipStreamCreate(&stream[i]);
  ...
  // Destroy Streams
  for (int i = 0; i < n_streams; ++i) hipStreamDestroy(stream[i]);
