...
int main(int argc, char *argv[]) {
  ...
  hipHostMalloc(&hA, N * sizeof(double));
  hipHostMalloc(&hB, N * sizeof(double));
  hipHostMalloc(&hC, N * sizeof(double));
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    hipSetDevice(i);
    hipMalloc(&dA[i], dec[i].len * sizeof(double));
    hipMalloc(&dB[i], dec[i].len * sizeof(double));
    hipMalloc(&dC[i], dec[i].len * sizeof(double));
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    hipSetDevice(i);
    // TODO: Copy data from host to device asynchronously (hA[dec[i].start] ->
    // dA[i], hB[dec[i].start] -> dB[i])
    hipMemcpyAsync(dA[i], &hA[dec[i].start], dec[i].len * sizeof(double),
                   hipMemcpyHostToDevice, strm[i]);
    hipMemcpyAsync(dB[i], &hB[dec[i].start], dec[i].len * sizeof(double),
                   hipMemcpyHostToDevice, strm[i]);
    ...
    hipMemcpyAsync(&hC[dec[i].start], dC[i], dec[i].len * sizeof(double),
