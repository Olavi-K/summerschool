...
#pragma omp target teams distribute parallel for collapse(2) \
    map(tofrom : prevdata[0 : field_size])                   \
    map(tofrom : currdata[0 : field_size])
...
void enter_data(Field& curr, Field& prev) {
  ...
  prevdata = curr.temperature.data();

  nx = curr.nx;
  ny = curr.ny;

#pragma omp target enter data map(to : currdata[0 : (nx + 2) * (ny + 2)], \
...
#pragma omp target exit data map(from : currdata[0 : (nx + 2) * (ny + 2)], \
                                     prevdata[0 : (nx + 2) * (ny + 2)])
}
