...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < localsize; i++) writevector[i] = i + 1 + localsize * my_id;

  h5_writer(my_id, writevector, localsize);

  h5_reader(my_id, readvector, localsize);

  for (i = 0; i < localsize; i++) assert(writevector[i] == readvector[i]);
...
void h5_writer(int my_id, int *localvector, int localsize) {
  ...
  dset_id = H5Dcreate(file_id, "data", H5T_NATIVE_INT, filespace, H5P_DEFAULT,
...
void h5_reader(int my_id, int *localvector, int localsize) {
  ...
  status = H5Dread(dset_id, H5T_NATIVE_INT, memspace, filespace, plist_id,
                   localvector);
