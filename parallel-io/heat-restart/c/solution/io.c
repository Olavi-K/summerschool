...
void write_field(field *temperature, int iter, parallel_data *parallel) {
  ...
  if (parallel->rank == 0) {
    ...
    for (p = 1; p < parallel->size; p++) {
      MPI_Recv(&tmp_data[0][0], temperature->nx * temperature->ny, MPI_DOUBLE,
               p, 22, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    ...
    for (i = 0; i < temperature->nx; i++)
      memcpy(tmp_data[i], &temperature->data[i + 1][1],
             temperature->ny * sizeof(double));
    MPI_Send(&tmp_data[0][0], temperature->nx * temperature->ny, MPI_DOUBLE, 0,
             22, MPI_COMM_WORLD);
  }
...
void read_field(field *temperature1, field *temperature2, char *filename,
  ...
  temperature1->data = malloc_2d(temperature1->nx + 2, temperature1->ny + 2);
  ...
  for (j = 0; j < ny + 2; j++) {
    ...
    temperature1->data[nx_local + 1][j] = temperature1->data[nx_local][j];
  }
...
void write_restart(field *temperature, parallel_data *parallel, int iter) {
  ...
  status = H5Dwrite(dset_id, H5T_NATIVE_DOUBLE, memspace, filespace, plist_id,
  ...
  attr_id = H5Acreate(dset_id, "Iteration", H5T_NATIVE_INT, attrspace,
...
void read_restart(field *temperature, parallel_data *parallel, int *iter) {
  ...
  status = H5Dread(dset_id, H5T_NATIVE_DOUBLE, memspace, filespace, plist_id,
                   &(temperature->data[0][0]));
